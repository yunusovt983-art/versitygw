...
func runGateway(ctx context.Context, be backend.Backend) error {
	...
	// Integrate metrics manager with backend if supported
	if metricsIntegration, ok := be.(interface{ SetVersityMetricsManager(interface{}) error }); ok {
		if err := metricsIntegration.SetVersityMetricsManager(metricsManager); err != nil {
			return fmt.Errorf("failed to integrate metrics manager with backend: %w", err)
		}
	}
